...
test('late registration upgrades existing tags and disposes connection listeners', async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    const { registerTemplate } = await import('/js/templates.js?v=0.7.0');
...
test('registration rejects invalid definitions and cannot replace an existing tag', async ({ page }) => {
  ...
  const errors = await page.evaluate(async () => {
    const { registerTemplate } = await import('/js/templates.js?v=0.7.0'); const template = document.createElement('template'); template.innerHTML = '<p data-tvw-text="message">Text</p>';
    const result = [];
