...
const parts = { button: 'buttons', notice: 'feedback', input: 'forms', card: 'cards' };
function render(changeComponent = false) {
  ...
  const node = document.createElement(type === 'button' ? 'button' : type === 'notice' ? 'div' : type === 'input' ? 'div' : 'article');
  if (type === 'button') {
  ...
  } else { node.className = 'tvw-card'; const heading = document.createElement('h3'), text = document.createElement('p'); heading.textContent = label.value; text.textContent = 'Supporting content goes here.'; node.append(heading, text); }
  node.dir = document.getElementById('play-rtl').checked ? 'rtl' : 'ltr';
  preview.replaceChildren(node); source.textContent = node.outerHTML; document.getElementById('play-status').textContent = '';
  document.getElementById('play-dependencies').textContent = `CSS: foundation.css + parts/${parts[type]}.css. JavaScript: none for this markup.`;
}
